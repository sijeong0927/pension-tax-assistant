...
def route_query(query: str, openai_client: Any, model: str) -> str | None:
    ...
    prompt = (
        "사용자의 질문이 세무서/국세청 직접 방문에 관한 것인지, 아니면 근로자의 연말정산 서류 제출/공제 한도에 관한 것인지 분류해 주세요.\n\n"
        "[분류 기준]\n"
        "- \"관할세무서\": 사용자가 세무서(또는 국세청)에 직접 방문(사업자등록, 민원실 서류 발급, 종합소득세 신고 직접 방문 등)하거나 방문 시 필요한 구비서류/준비물(위임장, 신분증 등)에 대해 질문하는 경우.\n"
        "- \"회사제출용\": 근로자가 회사(원천징수의무자)에 제출하는 연말정산 소득/세액공제 요건, 공제 한도, 소득공제 신고서, 금융회사 발급 증명서 제출에 대해 질문하는 경우.\n\n"
        f"[질문]: {query}\n\n"
        "반드시 JSON 형식으로 출력하세요:\n"
        "{\"intent\": \"관할세무서\"} 또는 {\"intent\": \"회사제출용\"}\n"
