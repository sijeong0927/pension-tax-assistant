...
코드에서도 검색 문서를 최대 8개, FAQ 1회 인덱싱 문서를 최대 100개, PDF 인덱싱 문서를
최대 1,000개 및 임베딩 요청을 최대 50회, 자동 재시도를 최대 1회로 제한합니다. 기본값은
각각 4개, 50개, 1,000개, 50회, 1회입니다.
...
스크립트는 다음 항목을 검사한 뒤 FAQ 가이드 1개와 FAQ 30개를 저장합니다.
...
삭제합니다. 임의 `--pdf` 파일은 출처를 검증된 공식 자료로 표시하지 않습니다.
